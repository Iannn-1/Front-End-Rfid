...
export default function ReportHistory({ refreshKey }: { refreshKey?: number }) {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      setHistory(res.data?.data?.history ?? res.data?.history ?? []);
    } catch {}
  ...
  const redownload = async (record: HistoryRecord) => {
    ...
    try {
      const res = await api.post("/reports/generate", { type: record.type });
  ...
  return (
    <div className={styles.card}>
      <div className={styles.cardHeader}>
        ...
        <button className={styles.button} onClick={fetchHistory} style={{ fontSize: 12, padding: "5px 12px" }}>
          ↻ Refresh
        </button>
      </div>
            ...
            {history.map(r => {
              ...
              return (
                <div key={r.id} style={{
                  ...
                  borderRadius: 12, border: "1px solid #f3f4f6", transition: "all 0.15s",
                }}>
                  ...
                  <button onClick={() => redownload(r)} disabled={redownloading === r.id} style={{
                    padding: "6px 12px", borderRadius: 8, border: "1px solid #e5e7eb",
                    background: redownloading === r.id ? "#f3f4f6" : "white",
                    cursor: redownloading === r.id ? "not-allowed" : "pointer",
                    fontSize: 12, fontWeight: 700, color: "#8b3b3b",
